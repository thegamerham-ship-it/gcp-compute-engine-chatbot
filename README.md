...
├── cloud_run/                   # ⚡ Google Cloud Run 서버리스 컨테이너 배포 패키지
│   ├── Dockerfile               # Cloud Run 표준 컨테이너 이미지 빌드 명세서
│   ├── server.py                # Cloud Run 환경 최적화 백엔드 서버 ($PORT, /health)
│   ├── deploy.py                # Python 기반 1-Click 자동 배포 파이프라인
│   ├── deploy.ps1 / deploy.sh   # Windows PowerShell / Linux Bash 1-Click 스크립트
│   ├── cloud_run_example.ipynb  # Cloud Run 배포 및 API 테스트 실습 노트북
│   ├── public/                  # 프론트엔드 정적 리소스 (동적 HTTPS 리다이렉트 적용)
│   └── README.md                # Cloud Run 아키텍처 및 상세 배포 가이드
├── deploy/
...
├── public/                      # 프론트엔드 정적 리소스 (Compute Engine용)
