/**
 * 관동지방 포켓몬 도감 (Pokédex Kanto ADC v2.1) & 간호순 누나 챗봇 프론트엔드
 * Supports:
 * - Google Cloud ADC(애플리케이션 기본 사용자 인증 정보) 상태 표시
...
(() => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.hasAuth) {
          ...
          statusText.textContent = data.hasAdc
            ? `도감 온라인 (ADC · ${data.projectId || 'GCP'})`
            : '도감 시스템 온라인';
          apiStatusChip.title = data.diagnostic || 'ADC 인증 정상 연동 중';
        } else {
          apiStatusChip.classList.remove('online');
          apiStatusChip.classList.add('offline');
          statusText.textContent = 'ADC 인증 필요';
          apiStatusChip.title = data.diagnostic || 'ADC 자격 증명이 감지되지 않았습니다.';
        }
      } else {
      ...
      apiStatusChip.title = '서버에 연결할 수 없습니다.';
    }
  }
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.detail || data.error || '답변을 생성하지 못했습니다.');
      }
      ...
      thoughtTitle.textContent = '도감 데이터 검색 완료 (ADC)';
      const thoughtBody = thoughtCard.querySelector('.thought-body');
