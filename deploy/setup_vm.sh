...
GCP_PROJECT=$(curl -s -H "Metadata-Flavor: Google" "http://metadata.google.internal/computeMetadata/v1/project/project-id" || echo "iceu-songpa21")
echo "🔑 [4/6] Fetching GEMINI_API_KEY from Secret Manager (projects/${GCP_PROJECT}/secrets/GEMINI_API_KEY)..."
SECRET_KEY=""
if command -v gcloud &> /dev/null; then
    SECRET_KEY=$(gcloud secrets versions access latest --secret=GEMINI_API_KEY --project="${GCP_PROJECT}" 2>/dev/null || echo "")
fi
...
try:
    ...
    name = 'projects/${GCP_PROJECT}/secrets/GEMINI_API_KEY/versions/latest'
